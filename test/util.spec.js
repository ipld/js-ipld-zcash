...
describe('IPLD format util API deserialize()', () => {
  it('should work correctly', async () => {
    const dagNode = await IpldZcash.util.deserialize(fixtureBlockHeader)
    return verifyBlock(dagNode, {
      version: 4,
  ...
  it('should error on an invalid block', async () => {
    ...
    return shouldThrow(IpldZcash.util.deserialize(invalidBlock))
  })
})
...
describe('IPLD format util API serialize()', () => {
  it('should round-trip (de)serialization correctly', async () => {
    const dagNode = await IpldZcash.util.deserialize(fixtureBlockHeader)
    const binaryBlob = await IpldZcash.util.serialize(dagNode)
    expect(binaryBlob).to.deep.equal(fixtureBlockHeader)
  })

  it('should error on an invalid internal representation', async () => {
    return shouldThrow(IpldZcash.util.serialize(invalidDagNode))
  })
})
...
describe('IPLD format util API cid()', () => {
  ...
  it('should error unknown hash algorithm', async () => {
    const dagNode = await IpldZcash.util.deserialize(fixtureBlockHeader)
    return shouldThrow(IpldZcash.util.cid(dagNode, { hashAlg: 'unknown' }))
  })

  it('should encode the CID correctly and ignore undefined options', async () => {
    const dagNode = await IpldZcash.util.deserialize(fixtureBlockHeader)
    return verifyCid1(
      dagNode,
      undefined,
  ...
  it('should error on an invalid internal representation', async () => {
    return shouldThrow(IpldZcash.util.cid(invalidDagNode))
  })
})

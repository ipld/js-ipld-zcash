...
describe('IPLD format resolve API resolve()', () => {
  it('should return the deserialized node if path is empty', () => {
    const value = IpldZcash.resolver.resolve(fixtureBlockHeader, '')
    expect(value.remainderPath).is.empty()
    expect(value.value).is.not.empty()
  })

  it('should return the version', () => {
    verifyPath(fixtureBlockHeader, 'version', 4)
  })

  it('should return the timestamp', () => {
    verifyPath(fixtureBlockHeader, 'timestamp', 1481233847)
  })

  it('should return the difficulty', () => {
    verifyPath(fixtureBlockHeader, 'difficulty', 477875354)
  })
  ...
  it('should return a link when parent is requested', () => {
    const value = IpldZcash.resolver.resolve(
      fixtureBlockHeader, 'parent')
    expect(value.remainderPath).is.empty()
  ...
  it('should return an error if block is invalid', () => {
    verifyError(
      invalidBlock, 'version', 'Zcash block header needs to be 1487 bytes')
  })
})

describe('IPLD format resolver API tree()', () => {
  it('should return only paths by default', () => {
    const value = IpldZcash.resolver.tree(fixtureBlockHeader)
    const paths = [...value]

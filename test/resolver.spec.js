...
describe('IPLD format resolver API resolve()', () => {
  it('should return the deserialized node if no path is given', async () => {
    const value = await IpldZcash.resolver.resolve(fixtureBlockHeader)
  ...
  it('should return the version', async () => {
    return verifyPath(fixtureBlockHeader, 'version', 4)
  })

  it('should return the timestamp', async () => {
    return verifyPath(fixtureBlockHeader, 'timestamp', 1481233847)
  })

  it('should return the difficulty', async () => {
    return verifyPath(fixtureBlockHeader, 'difficulty', 477875354)
  })
  ...
  it('should return the solution', async () => {
    return verifyPath(
      fixtureBlockHeader,
      'solution',
  ...
  it('should return the reserved', async () => {
    return verifyPath(
      fixtureBlockHeader,
      'reserved',
  ...
  it('should return a link when parent is requested', async () => {
    const value = await IpldZcash.resolver.resolve(fixtureBlockHeader, 'parent')
    expect(value.remainderPath).is.empty()
  ...
  it('should return a link and remaining path when parent is requested', async () => {
    const value = await IpldZcash.resolver.resolve(fixtureBlockHeader, 'parent/timestamp')
  ...
  it('should return a link when transactions are requested', async () => {
    const value = await IpldZcash.resolver.resolve(fixtureBlockHeader, 'tx/some/remainder')
...
describe('IPLD format resolver API tree()', () => {
  it('should return only paths by default', async () => {
    const value = await IpldZcash.resolver.tree(fixtureBlockHeader)
  ...
  it('should be able to return paths and values', async () => {
    const value = await IpldZcash.resolver.tree(fixtureBlockHeader, {values: true})
  ...
  it('should return an error if block is invalid', async () => {
    return shouldThrow(IpldZcash.resolver.tree(invalidBlock))
  })
})
...
const verifyPath = async (block, path, expected) => {
  const value = await IpldZcash.resolver.resolve(block, path)
  expect(value.remainderPath).is.empty()
  expect(value.value).to.deep.equal(expected)
}

const verifyError = async (block, path) => {
  return shouldThrow(IpldZcash.resolver.resolve(block, path))
}
